...
<!--    <xsl:variable name="total_id_text"><xsl:text>oai:crdo.vjf.cnrs.fr:</xsl:text><xsl:value-of select="substring-before($id, '_SOUND')"/></xsl:variable>-->
<xsl:variable name="total_id_text"><xsl:value-of select=".//oai:ListRecords/oai:record[oai:header/oai:identifier = $id]/oai:metadata/olac:olac/dcterms:isRequiredBy"/></xsl:variable>
	

<!-- ******************************************************** -->
<xsl:template match="/"><link rel="stylesheet" type="text/css" href="LacitoStyle.css"/>
<!--<link rel="stylesheet" type="text/css" href="http://lacito.vjf.cnrs.fr/pangloss/tools/LacitoStyle.css"/>-->


<xsl:if test="(.//oai:ListRecords/oai:record[oai:header/oai:identifier = $total_id]/oai:metadata/olac:olac) and (.//oai:ListRecords/oai:record[oai:header/oai:identifier = $total_id_text]/oai:metadata/olac:olac)">
<br/>
<ul>
<li><h1 style="color:#06C"><a href="#audio">Informations concernant la ressource audio</a></h1></li>
<li><h1 style="color:#06C"><a href="#annotations">Informations concernant les annotations</a></h1></li>
</ul>
<br/><br/>
</xsl:if>
    ...
    <xsl:for-each select=".//oai:ListRecords/oai:record[oai:header/oai:identifier = $total_id]/oai:metadata/olac:olac">
		<table class="meta">	
			<tr><td class="rubrique"></td></tr>
            <h1 align="center" style="color:#039"><a name="audio"><b>Ressource audio</b></a></h1>
			<xsl:call-template name="title"/>
			<xsl:call-template name="langue"/>
            <xsl:call-template name="identifier_Sound"/> 
	...
	<xsl:for-each select=".//oai:ListRecords/oai:record[oai:header/oai:identifier = $total_id_text]/oai:metadata/olac:olac">
    
    <br/><br/><p align="center"><xsl:text>--------------------------------</xsl:text></p>

		<table class="meta">	
			<tr><td class="rubrique"></td></tr>
            <h1 align="center" style="color:#039"><a name="annotations"><b>Annotations</b></a></h1>
			<xsl:call-template name="title"/>
			<xsl:call-template name="langue"/>
...
<xsl:template name="title">
	<xsl:if test="dc:title">
        ...
        <strong style="font-size:16px"> <xsl:apply-templates select="dc:title[1]"/>
        <xsl:if test="dcterms:alternative">
         / <xsl:apply-templates select="dcterms:alternative[1]"/>
        </xsl:if>
        </strong>
        </h2>
	</xsl:if>
	...
	<tr>
		...
		<td class="valeur">
			<xsl:for-each select="dc:subject[@xsi:type='olac:language']">
                ...
                <xsl:variable name="code_langue"><xsl:value-of select="@olac:code"/></xsl:variable>
                <xsl:apply-templates select="."/> (<a href="http://www.ethnologue.com/language/{$code_langue}" target="_blank"><xsl:value-of select="@olac:code"/></a>)<!--</a>-->
			</xsl:for-each>
		</td>
	</tr>
	<tr><td></td></tr>
...
<!--  affichage du lieu de l'enregistrement  -->
        ...
        <tr>
			...
			<td class="valeur">
                ...
                <xsl:for-each select="dc:description"> 
					(<xsl:value-of select="@xml:lang"/>)<xsl:apply-templates select="."/>
					<xsl:if test="position()!=last()"><br/></xsl:if>
				</xsl:for-each>
			</td>
		</tr>
...
<xsl:template name="coverage">
	<xsl:if test="(dc:cverage) or (dcterms:spatial[not(@xsi:type)]) or (dcterms:temporal)">
    <tr><td></td></tr>
		<tr>
			<td class="descripteur">Lieu / Place:</td>
			<td class="valeur">
...
<xsl:template name="identifier_Sound">
<tr><td></td></tr>
	<tr>
		<td class="descripteur">Fichier(s) audio / Recording(s): </td>
		<td class="valeur">
			<xsl:choose>
    ...
    <tr><td></td></tr>
	<tr>
		<td class="descripteur">Fichier(s) d'annotations : </td>
		<td class="valeur">
