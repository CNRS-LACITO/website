	...
	<xsl:template match="/">
		<result>
			...
			<xsl:for-each select="//oai:ListRecords/oai:record[oai:header/oai:identifier = $sound_id]/oai:metadata/olac:olac">
              ...
              <file_audio><xsl:value-of select="dc:identifier[starts-with(normalize-space(.), 'http://cocoon.tge-adonis.fr/data')][1]"/></file_audio>
             ...
             <file_xml><xsl:value-of select="//oai:ListRecords/oai:record[oai:header/oai:identifier = $text_id]/oai:metadata/olac:olac/dc:identifier[1]"/></file_xml>             
