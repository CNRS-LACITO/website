...
var ENDSHOW_MESG="Fin du diaporama, cliquez pour le fermer.", SCREEN_MODE="Frames", gIsEndShow=0, NUM_VIS_SLDS=30, SCRIPT_HREF="script.js", FULLSCR_HREF="fullscreen.htm";
...
var sNext="Suivante",sPrev="Précédente ",sEnd="Mettre fin au diaporama",sFont="Arial", alwaysOn = false
function ShowMenu()
{
	BuildMenu();
...
function createCM() {
  if ( base.IsFullScrMode() ) {
	...
	initMe( PPTSld.Fobj, clr, "Suivante" );
	PPTSld.Fobj.onclick = M_GoNextSld;

	initMe( PPTSld.Bobj, clr, "Pr?c?dente" );
	PPTSld.Bobj.onclick = M_GoPrevSld;

	initMe( PPTSld.Eobj, clr, "Mettre fin au diaporama");
...
var g_baseURL = gtmpstr.substr(0, gtmpstr.lastIndexOf("/") ) + "/" + "mission_hiver_2007_fichiers";
var g_showoutline = 1;
  ...
  if ( (msie || isnav ) && ( (os < 0 && ver >= 4 ) || ( os >= 0 && ver >= 5 ) || (os >=0 && msie < 0 && ver >= 4 ) )  ){
	if ( msie >= 0 )  {
		if ( isIEFiles > 0 )
			...
			/*if ( IsFramesMode() )
				StatusPlay("Cette présentation est optimisée pour des versions antérieures de votre navigateur. Étant donné que vous utilisez une version plus récente de Microsoft Internet Explorer ou de Netscape Navigator, optimisez cette présentation pour pouvoir profiter des capacités avancées de votre navigateur."); */
			jpegArray( 30 );
		}
		else
			window.location.replace(  "mission_hiver_2007_fichiers/error.htm" );
	}
	else {
	...
	if ( IsFramesMode() && !isWebTV() )
		StatusPlay("Il est possible que votre navigateur ne puisse pas afficher correctement le contenu de cette présentation. Elle est optimisée pour des versions plus récentes de  Microsoft Internet Explorer ou de Netscape Navigator.");
	*/
	if ( isFLATFiles <= 0 ) {
	/*	if ( IsFramesMode() )
			window.alert("Il est possible que votre navigateur ne puisse pas afficher correctement le contenu de cette présentation. Elle est optimisée pour des versions plus récentes de  Microsoft Internet Explorer ou de Netscape Navigator." );
	*/
		window.location.replace(  "mission_hiver_2007_fichiers/error.htm" );
	}		
	/*
