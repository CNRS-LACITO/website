...
<!--<xsl:variable name="id_t"><xsl:value-of select="ancestor::dcterms:isRequiredBy, 'oai:crdo.vjf.cnrs.fr:')"/></xsl:variable>-->
	...
	<xsl:for-each select="dcterms:isRequiredBy">
				...
				<xsl:variable name="id_t"><xsl:value-of select="."/></xsl:variable>
                <xsl:variable name="id_text"><xsl:value-of select="substring-after($id_t,'oai:crdo.vjf.cnrs.fr:')"/></xsl:variable>
